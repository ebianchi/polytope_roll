...
title = (
    f"Non-convex Trajectory Optimization, {blurb}, Double Pivot{freq}"
    if USE_NON_CONVEX
    else f"Big M Trajectory Optimization, {blurb}, Double Pivot{freq}"
)
file_title = (
    f"traj_opt_{short}_double{f_short}"
    if USE_NON_CONVEX
...
x_goal = np.array([0.0, 0.0, 0.0, -4.5, 1.0, np.pi])
...
traj_opt.generate_visuals_from_latest_report(
    file_title=file_title, title=title, save=SAVE_OUTPUT
)
breakpoint()
