...
class iLQR:
    ...
    def __init__(
        ...
        self.decay = (
            solver_params.decay
        )  # tune this to control decay of learning rate.
    ...
    def get_linearized_discrete_dynamics(self, state, action):
        ...
        A, B_gen, C_mat, _, G_mat, H_mat, J_mat, _, pmat = (
            self.lcs.get_lcs_terms()
        )
        B = B_gen @ pmat
    ...
    def backward_pass(self, xx, uu):
        ...
        V_xp1 = self.grad_terminal_cost(
            final_state
        )  # jacobian of value func at the next state
        V_xxp1 = self.hess_terminal_cost(
            final_state
        )  # hess of value func at the next state
    ...
    def animate(self, init_state, controls):
        ...
        vis_utils.animation_gif_polytope(
            self.lcs.system.params.polytope,
            states,
            gif_name,
            self.lcs.system.params.dt,
    ...
    def calculate_optimal_trajectory(self, x, uu_guess):
        ...
        while i < self.max_iter:
            ...
            while learning_rate >= 0.05:
                xx_new, uu_new = self.forward_pass(
                    self.states, self.actions, dd, KK, learning_rate
                )
            ...
            if learning_rate < 0.05:
                print(
                    "Step size has become too small to move. Ending optimization"
                )
                break
            if i % 10 == 0 and i > 1:
