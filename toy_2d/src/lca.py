...
class admm_lca(object):
    ...
    def __init__(self, traj_opt_object, x_init, u_init, x_goal, rho):
        ...
        # TODO: allow planning states to be subset of true states.
    ...
    def solve_reference_gurobi(self):
        ...
        for i in range(self.N):
            ...
            dual_err = r[i, :] - self.x[i, :]@self.Tr + self.vr[i, :]
            obj += (self.rho/2) * dual_err @ dual_err

            # TODO: Left off here, there's a size issue
            dual_err = a[i, :] - self.u[i, :] + self.vu[i, :]
            obj += (self.rho/2) * dual_err @ dual_err

            dual_err = gamma[i, :] - self.lam[i, :] + self.vgamma[i, :]
            obj += (self.rho/2) * dual_err @ dual_err
