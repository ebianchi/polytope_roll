...
class admm_lca(object):
    ...
    def __init__(self, traj_opt_object, x_init, x_goal, rho, tol, T):
        ...
        self.x_init = x_init
        self.x_goal = x_goal
        self.optimization_time_limit = traj_opt_object.params.optimization_time_limit
    ...
    def construct_LCS_terms_from_inputs(self):
        ...
        self.traj_opt_object._construct_LCS_terms_from_inputs(self.u, self.x_init)
    ...
    def solve_reference_gurobi(self):
        ...
        a = GurobiModelHelper.create_us(
            model, lookahead=self.N, nu=self.m, input_limit=input_limit
        )
        gamma = GurobiModelHelper.create_lambdas(
            model, lookahead=self.N, p=self.p, k=self.k
        )
        y = GurobiModelHelper.create_ys(model, lookahead=self.N, p=self.p, k=self.k)
    ...
    def solve_control_gurobi(self):
        ...
        u = GurobiModelHelper.create_us(
            model, lookahead=self.N, nu=self.m, input_limit=input_limit
        )
        lam = GurobiModelHelper.create_lambdas(
            model, lookahead=self.N, p=self.p, k=self.k
        )
        y = GurobiModelHelper.create_ys(model, lookahead=self.N, p=self.p, k=self.k)
        ...
        # 2. 0 <= λ_t, G_t*x_t + H_t*u_t + J_t*λ_t + l_t >= 0, P_t*x_t + Q_t*u_t + S_t*λ_t <= 0
        model = GurobiModelHelper.add_complementarity_constr(
            model,
            lookahead=self.N,
            use_big_M=use_big_M,
    ...
    def plot_trajectories(self, loop: int = 0, create: bool = False, save: bool = True):
    ...
    def run_admm(self):
        ...
        while err >= self.tol:
            ...
            err = (
                np.trace((self.r - self.x @ self.Tr).T @ (self.r - self.x @ self.Tr))
                + np.trace((self.a - self.u).T @ (self.a - self.u))
                + np.trace((self.gamma - self.lam).T @ (self.gamma - self.lam))
            )
...
def main():
    ...
    def complex_car(x, u, t):
        ...
        return jnp.array(
            [
                v * jnp.cos(psi) - 0.01 * w * jnp.sin(psi),
                v * jnp.sin(psi) + 0.01 * w * jnp.cos(psi),
                ul - 0.01 * w**2,
                w,
                ua,
            ]
