...
x_goal = np.array([0.0, 0.0, 0.0, 2.0, 1.0, -np.pi / 2])
...
sim_system_params = TwoDimensionalSystemParams(
    dt=DT_SIM, polytope=polytope, mu_control=0.5
)
...
traj_opt_system_params = TwoDimensionalSystemParams(
    dt=DT_TRAJ_OPT, polytope=polytope, mu_control=0.5
)
traj_opt_system = TwoDSystemMagOnly(
    traj_opt_system_params, CONTACT_LOC, CONTACT_ANGLE
)
...
for _ in range(LOOPS):
    ...
    try:
        ...
        model.addConstrs(
            (x_errs[i, :] == xs[i + 1, :] - x_goal for i in range(LOOKAHEAD)),
    ...
    control_input = max(us.X[0], np.array([0.0]))
...
title = (
    "Non-convex Trajectory Optimization"
    if USE_NON_CONVEX
    else "Big M Trajectory Optimization"
)
...
vis_utils.animation_gif_polytope(
    polytope, states, file_title, DT_SIM, controls=controls, save=SAVE_OUTPUT
)
